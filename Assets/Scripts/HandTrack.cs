using System;
using System.Collections;
using System.Collections.Generic;
using UnityEngine;

//! The logic for the individual hands
/*!
  Every frame, checks for the distance and rotation difference. If it falls under the set allowance, move this hand to the next frame and update icon line accordingly.
*/
public class HandTrack : MonoBehaviour
{
  [Tooltip("The distance differece allowed to move on to the next frame")]
  //! [Input] The distance differece allowed to move on to the next frame
  public float distAllowance = 0.025f;

  
  [Tooltip("The angle difference allowed to move on to the next frame")]
  //! [Input] The angle difference allowed to move on to the next frame
  public float angleAllowance = 20f;
  

  [Tooltip("Which hand is this (0 = left, 1 = right)")]
  [Range(0, 3)]
  //! Which hand the script is currently attatched to (0 = left, 1 = right)
  public int handIndex = 0;

  //! [Input] Respective icon line script
  [Tooltip("The respective icon line draw script")]
  public LineDraw iconLine;

  //! [Input] The UI selection line
  [Tooltip("The UI selection line")]
  public GameObject UILine;

  public ExportData exportData;

  //! Current frame
  private int currFrame = 0;

  private bool print = true;
  private GhostHands ghostHands;
  private bool first = true;
  private float timer = 0f;

  void Start()
  {
    ghostHands = this.GetComponent<GhostHands>();
    
    if (iconLine == null) {
      Debug.LogError("Icon line is not set for hand " + handIndex);
    }
  }

  void Update()
  {
    if (Globals.start) {
      UILine.SetActive(false);
    }

    // Check if initial load is done and video is paused
    if (Globals.start && Globals.paused) {
      // Set current frame to start if not correct
      if (currFrame < Globals.traces[Globals.currSet][handIndex].Positions.Count) {
        currFrame = 0;
        if (Globals.animated) {
          iconLine.UpdateLine(currFrame);
        }
      }
      else { return; }
      timer += Time.deltaTime;

      // +/- is to offset to where the line center is so the line point and hand are on the same coordinate system
      Vector3 original = Globals.traces[Globals.currSet][handIndex].Positions[currFrame];
      Vector3 check = new Vector3(
        original.x,
        original.y,
        original.z + Globals.ghostOffset
      );

      // if (first) {
      //   Debug.Log("Original position: " + original);
      //   Debug.Log("Offset position: " + iconLine.offset);
      //   Debug.Log("Check position: " + check);
      //   Debug.Log("This position: " + this.transform.position);
      //   first = false;
      // }

      // Check distance between hand and correct position
      float dist = Vector3.Distance(check, ghostHands.sceneGhost.transform.localPosition);
      if (dist <= distAllowance) {

        // Get distance between current angle and correct angle
        // Quaternion angleCheck = Quaternion.Inverse(ghostHands.sceneGhost.transform.rotation) * Globals.userHands[handIndex].Rotations[currFrame];
        // Use euler angles to make comparison easier
        float angleDist = Vector3.Distance(ghostHands.sceneGhost.transform.eulerAngles, Globals.userHands[handIndex].Rotations[currFrame]);

        // Correct for angles that end up over 360
        // if (angleDist.x > 180) { angleDist.x -= 360; }
        // if (angleDist.y > 180) { angleDist.y -= 360; }
        // if (angleDist.z > 180) { angleDist.z -= 360; }

        // if (Math.Abs(angleDist.x) <= angleAllowance && 
        //     Math.Abs(angleDist.y) <= angleAllowance && 
        //     Math.Abs(angleDist.z) <= angleAllowance) 
        // {
        if (angleDist <= angleAllowance) {
          // Save hand position & rotation for replay
          Globals.userHands[handIndex].Positions.Add(ghostHands.sceneGhost.transform.localPosition);
          Globals.userHands[handIndex].Rotations.Add(ghostHands.sceneGhost.transform.eulerAngles);

          // Next frame
          // Debug.Log("Added hand " + handIndex + ": " + Globals.userHands[handIndex].Positions.Count + " " + ghostHands.sceneGhost.transform.localPosition);
          Globals.userHands[handIndex].Timestamps[currFrame] = timer;
          // Debug.Log(handIndex + ": " + Globals.armCheck[handIndex] + "/" + Globals.gestures[Globals.currGest].End);
          
          // Turn off current icon (if multiple traces)
          if (!Globals.animated) {
            iconLine.UpdateLine(currFrame, false);
          }
          
          currFrame++;
          
          // exportData.GetPosAccuracy(handIndex, currFrame, distAllowance);
          // exportData.GetRotAccuracy(handIndex, currFrame, angleAllowance);
          
          // Show icon for next frame
          if (Globals.animated) {
            iconLine.UpdateLine(currFrame);
          }
        }
      }
    }
  }

  //! Move hand back to the start of the current gesture. Triggered by rewind.
  public void ResetHand() {
    currFrame = 0;
    Globals.currFrame = 0;
  }
}
